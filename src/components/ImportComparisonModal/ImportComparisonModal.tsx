...
function ComparisonTablePreview({ result }: { result: OrkgComparisonResult }) {
  ...
  if (!studies.length) {
    return <div className="text-xs text-muted italic p-2">No studies found.</div>;
  }
      ...
      <table className="w-full text-xs text-left border-collapse table-fixed">
        <thead className="bg-surface-secondary font-semibold border-b">
          <tr>
            ...
            {studies.map((s, idx) => (
              <th key={s.id || idx} className="p-2 border-r w-56 font-semibold text-foreground" title={s.name}>
        ...
        <tbody className="divide-y">
          {props.map((prop) => (
            <tr key={prop} className="hover:bg-surface-secondary/40">
              ...
              {studies.map((s, idx) => {
                const val = s.properties[prop] || s.properties[prop.toLowerCase()] || '—';
                return (
                  <td key={s.id || idx} className="p-2 border-r text-xs text-muted align-top whitespace-normal">
                    {val}
                  </td>
                );
              })}
...
export default function ImportComparisonModal({
  ...
  const [comparisonResults, setComparisonResults] = useState<OrkgComparisonResult[]>([]);
  ...
  const handleFetch = async () => {
    ...
    if (!ids.length) {
      setError('No valid ORKG comparison links or identifiers found. Please check your input.');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!results.length) {
        setError('Could not retrieve any comparison tables from the provided links. Please verify the IDs.');
      } else {
  ...
  const totalStudies = comparisonResults.reduce((acc, r) => acc + r.contributionCount, 0);
        ...
        <Modal.Dialog className="max-w-4xl">
          ...
          <Modal.Body className="space-y-4">
            <p className="text-sm text-muted">
              Paste one or multiple ORKG comparison URLs or IDs (one per line or separated by commas):
            </p>

            <div className="space-y-2">
              <TextArea
                rows={3}
                placeholder={"https://orkg.org/comparisons/R1587225\nhttps://orkg.org/comparisons/R1587217\nR1587219"}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold text-foreground">
                    Loaded Comparisons ({comparisonResults.length}) — {totalStudies} total studies
                  </span>
                  <Button
                    size="sm"
                    variant="ghost"
                ...
                <ScrollShadow className="max-h-80 overflow-y-auto space-y-2 pr-1">
                  {comparisonResults.map((res) => (
                    <Card key={res.id} variant="secondary" className="p-3 space-y-2">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        <div className="flex items-center gap-1">
                          <Button
                            ...
                            onPress={() =>
                              setActivePreviewId(activePreviewId === res.id ? null : res.id)
                            }
                          >
                            <FontAwesomeIcon
          ...
          <Modal.Footer className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              ...
              >
                <FontAwesomeIcon icon={comparisonResults.length ? faCheck : faDownload} />
                <span>
                  Import & Synthesize {comparisonResults.length > 0 ? `(${comparisonResults.length})` : ''}
                </span>
              </Button>
            </div>
          </Modal.Footer>
