...
import { Alert, Button, Modal, ScrollShadow, Spinner, TextArea } from '@heroui/react';
...
type Props = {
  ...
  onQueryComplete: (query: string, nlResponse: string, sparqlQuery: string) => void;
};

export default function OrkgNlQueryModal({ isOpen, onOpenChange, onQueryComplete }: Props) {
  ...
  const handleQuery = async () => {
    ...
    try {
      ...
      onQueryComplete(queryText.trim(), result.naturalLanguageResponse, result.sparqlQuery);
      onOpenChange(false);
    } catch (err: any) {
      setError(err?.message ?? 'Failed to query ORKG. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <Modal.Dialog className="max-w-xl">
          ...
          <Modal.Body className="space-y-4">
            <p className="text-sm text-muted">
              Ask anything about research in ORKG. Your question is converted to SPARQL, executed
              against the ORKG triplestore, and returned as a structured summary.
            </p>
          ...
          <Modal.Footer className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              ...
              >
                {isLoading ? <Spinner size="sm" color="current" /> : <FontAwesomeIcon icon={faSearch} />}
